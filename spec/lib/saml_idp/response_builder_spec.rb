  ...
  describe ResponseBuilder do
    ...
    it "builds a legit raw XML file" do
      Timecop.travel(Time.zone.local(2010, 6, 1, 13, 0, 0)) do
        xml_document = Nokogiri::XML(subject.raw)
        xml_document.xpath("/samlp:Response/@ID").first.value.should eq "_abc"
        xml_document.xpath("/samlp:Response/@Destination").first.value.should eq "http://sportngin.com"
        xml_document.xpath("/samlp:Response/@Consent").first.value.should eq "urn:oasis:names:tc:SAML:2.0:consent:unspecified"
        xml_document.xpath("/samlp:Response/@InResponseTo").first.value.should eq "134"
        xml_document.xpath("/samlp:Response/@IssueInstant").first.value.should eq "2010-06-01T13:00:00Z"
        xml_document.xpath("/samlp:Response/@Version").first.value.should eq "2.0"
